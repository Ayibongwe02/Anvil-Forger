...
# Copy app files (order: dependencies first, then source for better caching)
COPY requirements.txt .
COPY app.py .
COPY src/ ./src/
...
CMD ["gunicorn", "-w", "1", "-b", "0.0.0.0:5000", "--timeout", "300", "--access-logfile", "-", "--error-logfile", "-", "app:app"]
