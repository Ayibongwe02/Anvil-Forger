...
from src.constants import TASK_KINDS, TABULAR_ALGORITHMS_CLASSIFICATION, \
    TABULAR_ALGORITHMS_REGRESSION, IMAGE_ALGORITHMS, ALGORITHM_LABELS, UNIVERSAL_EXPORT_SUPPORTED, \
    IMPORT_TASK_TYPES
...
def forgot_password():
    if request.method == "POST":
        ...
        token, _ = auth.request_password_reset(email)
        # Always show a neutral message when the email is unknown (no enumeration).
        if not token:
            flash(
                "If that email is registered, a reset link has been prepared. "
                "Check your inbox or ask your team admin if you do not receive one.",
                "success",
            )
            return redirect(url_for("login"))

        reset_url = url_for("reset_password", token=token, _external=True)
        from src.email_util import smtp_configured, send_password_reset_email, EmailSendError

        if smtp_configured():
            try:
                send_password_reset_email(email.lower().strip(), reset_url)
                flash(
                ...
                # Fail open for internal deploys: still surface the link so the user is not stuck.
                flash(
                    f"Could not send email ({e}). Use this one-time reset link instead "
                    f"(valid 1 hour): {reset_url}",
                    "warning",
                )
        else:
            # No SMTP — internal-tool mode: show the link once in the UI.
            flash(
                f"Password reset link generated (valid 1 hour). "
...
def reset_password(token):
    ...
    if request.method == "POST":
        ...
        error = auth.reset_password_with_token(token, new_password)
        if error:
            flash(error, "error")
            return render_template("reset_password.html", token=token)
...
def change_password():
    if request.method == "POST":
        ...
        error = auth.change_password(user["id"], current, new_password)
        if error:
            flash(error, "error")
            return render_template("change_password.html")
...
def project_detail(project_id):
    ...
    if project["task_kind"] == "tabular":
        algo_choices = sorted(set(TABULAR_ALGORITHMS_CLASSIFICATION + TABULAR_ALGORITHMS_REGRESSION))
    else:
        algo_choices = IMAGE_ALGORITHMS
